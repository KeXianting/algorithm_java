...
public class LogAnalysis {
    ...
    public void creatLog(File log, long logNums) throws Exception{
        ...
        for (int i = 0; i < logNums; i++) {
            ...
            if((i+1) % 1000 == 0){
                bw.flush();//将缓存中的输出流（字节流，字符流等）强制输出到文件
            }
        }
        bw.flush();
        fw.close();
    ...
    public void splitLog(File logflie,int fileNums) throws Exception{
        ...
        for(int i=0;i<fileNums;i++){
            File file = new File("/Users/kentorvalds/Documents/studyScource/算法/temp_file/logSplit/"+ i + ".txt");
        ...
        while(ip != null){
            ...
            List<String> list = dataMap.get(fileNum);//每一个文件的统计
            list.add(ip + "\n");
            if(list.size() % 1000 == 0){
